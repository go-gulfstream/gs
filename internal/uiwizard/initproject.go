...
import "github.com/go-gulfstream/gs/internal/schema"
...
func (p *Project) Run() (err error) {
	p.name, err = inputControl("Name", "", true)
	if err != nil {
		return err
	}
	p.streamName, err = inputControl("StreamName", "", true)
	if err != nil {
		return err
	}
	p.gomodules, err = inputControl("GoModules", "", true)
	if err != nil {
		return err
	}
	p.commandPkg, err = inputControl("GoCommandPkg", "", true)
	if err != nil {
		return err
	}
	p.eventPkg, err = inputControl("GoEventPkg", "", true)
	if err != nil {
		return err
	}
	p.streamPkg, err = inputControl("GoStreamPkg", "", true)
	if err != nil {
		return err
	}
	return nil
